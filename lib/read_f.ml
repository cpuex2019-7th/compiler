...
let rec read_float _ =
  let rec read_float_min x =
    let a = read () in
    if a <= 32 then (-1.0 *.  x)
    else if a = 46 then read_float_min_dec x 0.1 
    else read_float_min ((x *. 10.0) +. (float_of_int (a - 48))) in
  let rec read_float_intl x =
    let a = read () in
    if a <= 32 then x
    else if a = 46 then read_float_dec x 0.1 
    else if a = 45 then read_float_min x
    else read_float_intl ((x *. 10.0) +. (float_of_int (a - 48)))
